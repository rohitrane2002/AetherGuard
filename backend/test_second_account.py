...
def test_pydantic():
    with SessionLocal() as db:
        try:
            current_user = db.execute(select(User).where(User.email == "test_debug25@aetherguard.dev")).scalar_one_or_none()
            if not current_user:
                print("User not found!")
                return

            print("Got user:", current_user.email)
            subscription = db.execute(
                select(Subscription).where(Subscription.user_id == current_user.id).order_by(Subscription.id.desc())
            ).scalar_one_or_none()
            
            print("Got subscription:", getattr(subscription, 'plan', 'None'))
            ...
            print("Got workspace counts")
            notifications_feed = list_notifications(db, current_user, limit=8)
            print("Got notification feed:", len(notifications_feed))
