...
- Set `STRIPE_BILLING_PORTAL_RETURN_URL` so users land back on the account center after managing billing in Stripe.
- If `OPENROUTER_API_KEY` or `OPENAI_API_KEY` is set, `/chat` uses the configured model. Otherwise it returns the fallback assistant message.
- For AetherGuard’s copilot, `anthropic/claude-sonnet-4-6` is the recommended default on OpenRouter for vulnerability explanations and secure Solidity rewrites.
